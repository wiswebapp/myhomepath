<?php
  ...
  $funcUrl = $methodUrl."category";
  $addDataUrl = $methodUrl."add-category";
  $editDataUrl = $methodUrl."edit-category";
  $subCatUrl = $methodUrl."sub-category/";

  $pageData = $data['pagedata']['data'];
  $tconfig = $data['pagedata']['tconfig'];
  $count = (count($pageData) > 0) ? count($pageData) : 0;

  $countAll = ($data['pagedata']['count'] > 0) ? $data['pagedata']['count'] : 0;
                            ...
                            <tr>
                              ...
                              <th>
                                <select class="form-control" name="status">
                                  ...
                                  <option <?php if(trim(@$_GET['status']) == "Yes")echo 'selected';?> value="Active">Active</option>
                                  <option <?php if(trim(@$_GET['status']) == "No")echo 'selected';?> value="InActive">InActive</option>
                                </select>
                              </th>
                              <th>
                                    ...
                                    <?php 
                                      if($count > 0):
                                        ...
                                        for ($i=0; $i < $count ; $i++) {
                                          ...
                                          $catId = $pageData[$i]['id'];
                                          if($pageData[$i]['status'] == 'Active'){
                                            $status = "<span class='label label-success'>Active</span>";
                                          }elseif($pageData[$i]['status'] == 'InActive'){
